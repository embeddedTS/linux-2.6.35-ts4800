...
static unsigned long ioaddr=0xb0012000;
...
static int __init tslcd_init(void)
{
  ...
  vreg = (unsigned short *)ioremap(ioaddr,4096);
  if (vreg == 0) {
  ...
  tY = vreg + 0x1;
  if (!hardcode) {
    use_qcalib = load_calibration(vreg+(0x86 / sizeof(unsigned short)),
  ...
  //tslcd_dev->evbit[0] = BIT(EV_KEY) | BIT(EV_ABS);
  //tslcd_dev->keybit[LONG(BTN_TOUCH)] = BIT(BTN_TOUCH);
...
module_param(ioaddr, ulong, 0644);
MODULE_PARM_DESC(ioaddr, "IO Address of Touchscreen controller");
