...
static volatile unsigned short *sysconregs;
static inline unsigned short PEEK16(unsigned long addr);
static inline void POKE16(unsigned long addr, unsigned short dat);
#define peek16(adr) PEEK16((unsigned long)&sysconregs[(adr)/2])
#define poke16(adr, val) POKE16((unsigned long)&sysconregs[(adr)/2],(val))
#endif
...
MODULE_PARM_DESC(macaddr, "Mac Address to be assigned.");
*/

#define MAX_HW_CARDS 4
static char *macaddr[MAX_HW_CARDS]={NULL,NULL,NULL,NULL};
module_param_array(macaddr, charp, NULL, 0);
MODULE_PARM_DESC(macaddr, "Mac Address to be assigned.");
...
static void smsc95xx_init_mac_address(struct usbnet *dev)
{
#ifdef CONFIG_MACH_TS48XX
   {
      ...
      if (dev->net->ifindex>0)
         return;
      if (port_num>=MAX_HW_CARDS)
	return;

      if (macaddr[port_num]==NULL) {

         sysconregs = (unsigned short *)ioremap_nocache(0xb0010000, 4096);
         read_tagmem(tagmem);
         dev->net->dev_addr[0] = 0x00;
         dev->net->dev_addr[1] = 0xd0;
         dev->net->dev_addr[2] = 0x69;

         if ((tagmem[0] & 0xFFFFFF00) == 0xFFFFFF00) {
            unsigned long delay;
            printk("TS-48XX tagmem blank; Selecting a random MAC...");
            /*for(i=3; i<6; i++) {
               delay = jiffies + HZ;
               poke16(0x0e, 1);  // Feed the WDT for 2.7 seconds
               while(time_before(jiffies, delay)); // wait 1 second
               dev->net->dev_addr[i] = peek16(0x0c);
               printk(".");
            }*/
            printk("\n");
            random_ether_addr(dev->net->dev_addr);
         }  else {
            dev->net->dev_addr[3] = tagmem[0] >> 24;
            dev->net->dev_addr[4] = tagmem[0] >> 16;
            dev->net->dev_addr[5] = port_num+1 + (tagmem[0] >> 8);
            if (dev->net->dev_addr[5]==0) /* we wrapped from 0xFF to 0*/
               dev->net->dev_addr[4]++;
            if (dev->net->dev_addr[4]==0)
               dev->net->dev_addr[3]++;
         }
         //poke16(0x0e, 2);  /* Feed the WDT for 10 seconds */
         iounmap(sysconregs);
         ...
         int r;
         r = sscanf(macaddr[port_num], "%x:%x:%x:%x:%x:%x",  &a, &b, &c, &d, &e, &f);

         if (r!=6) {
            printk("%s: Invalid MAC address\n", SMSC_CHIPNAME);
         } else {
          dev->net->dev_addr[0]=a;
          dev->net->dev_addr[1]=b;
          dev->net->dev_addr[2]=c;
          dev->net->dev_addr[3]=d;
          dev->net->dev_addr[4]=e;
          dev->net->dev_addr[5]=f;
         }
      }
...
void sspi_cmd(unsigned int cmd) {
  unsigned int i;
   ...
   // pulse CS#
   mdelay(10);
   sysconregs[0x14/2] = 0x2;
   mdelay(10);
   sysconregs[0x14/2] = 0x0;
   mdelay(10);

   for (i = 0; i < 32; i++, cmd <<= 1) {
      mdelay(10);
      if (cmd & 0x80) {
         sysconregs[0x14/2] = 0x4;
         mdelay(10);
         sysconregs[0x14/2] = 0xc;
      } else {
         sysconregs[0x14/2] = 0x0;
         mdelay(10);
         sysconregs[0x14/2] = 0x8;
      }
   }
   mdelay(10);
}
...
static int read_tagmem(unsigned int *tagmem) {
   ...
   for (j = 0; j < 20; j++) {
      ...
      for (ret = 0x0, i = 0; i < 32; i++) {
         ...
         sysconregs[0x14/2] = 0x8;
         /*mdelay(10);*/
         ret = ret << 1 | (sysconregs[0x14/2] & 0x1);
      }
      /*printk("%d %x %x\n",j,ret,sysconregs[0x14/2]);*/
      tagmem[j] = ret;
   }
