...
#define IO_ADDRESS(x)   \
	...
	(((x) >= (unsigned long)AIPS2_BASE_ADDR) && \
	  ((x) < (unsigned long)AIPS2_BASE_ADDR + AIPS2_SIZE)) ? \
	   AIPS2_IO_ADDRESS(x) : \
        (((x) >= (unsigned long)FPGA_BASE_ADDR) && \
          ((x) < (unsigned long)FPGA_BASE_ADDR + FPGA_SIZE)) ? \
           FPGA_IO_ADDRESS(x) : \
	0xDEADBEEF)
...
#define FPGA_IO_ADDRESS(x)  \
        (((x) - FPGA_BASE_ADDR) + FPGA_BASE_ADDR_VIRT)
#endif
...
#ifdef CONFIG_MACH_TS48XX
#define FPGA_BASE_ADDR		0xB0010000
#define FPGA_BASE_ADDR_VIRT	0xF9010000
#define FPGA_SIZE               SZ_1M
#endif
