...
static int mc13892_regulator_init(struct mc13892 *mc13892)
{
	...
	mc13892_register_regulator(mc13892, MC13892_GPO4, &gpo4_init);
        ...
        //RONALD: enable PMIC and change SW3 to 1.3V
        if (1)
	{
		int ret;
                struct regulator *gp_core;
                struct pad_desc cspi1_sclk = MX51_PAD_CSPI1_SCLK__CSPI1_SCLK;
		struct pad_desc cspi1_miso = MX51_PAD_CSPI1_MISO__CSPI1_MISO;
		struct pad_desc cspi1_mosi = MX51_PAD_CSPI1_MOSI__CSPI1_MOSI;
		struct pad_desc cspi1_sso = MX51_PAD_CSPI1_SS0__CSPI1_SS0;
		struct pad_desc cspi1_sclk_gpio = MX51_PAD_CSPI1_SCLK__GPIO_4_27;
                struct pad_desc cspi1_miso_gpio = MX51_PAD_CSPI1_MISO__GPIO_4_23;
                struct pad_desc cspi1_mosi_gpio = MX51_PAD_CSPI1_MOSI__GPIO_4_22;
                struct pad_desc cspi1_sso_gpio = MX51_PAD_CSPI1_SS0__GPIO_4_24;

                mxc_iomux_v3_setup_pad(&cspi1_sclk);
		mxc_iomux_v3_setup_pad(&cspi1_miso);
		mxc_iomux_v3_setup_pad(&cspi1_mosi);
		mxc_iomux_v3_setup_pad(&cspi1_sso);
                gp_core = regulator_get(NULL, "SW3");
                ret = regulator_set_voltage(gp_core, 1250000, 1250000);
                if (ret < 0)
                        printk(KERN_DEBUG "COULD NOT SET GP VOLTAGE!!!\n");
		mxc_iomux_v3_setup_pad(&cspi1_sclk_gpio);
		mxc_iomux_v3_setup_pad(&cspi1_miso_gpio);
		mxc_iomux_v3_setup_pad(&cspi1_mosi_gpio);
		mxc_iomux_v3_setup_pad(&cspi1_sso_gpio);
        }
